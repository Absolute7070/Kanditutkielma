program main
    ...
    integer, parameter :: numberOfIterations = 1000, numberOfConfig = 1000, numberOfParticles = 2     ! number of iterations across whole configuration space and number of configurations 
    ...
    real(rn), dimension(numberOfParams) :: parameterValues = [ 2.0, 0.5] 
    ...
    integer, parameter :: numberOfParallel = 1 ! number of parallel subconfiguration spaces for Metropolis
    integer :: iseed 
    ...
    allocate(energyInConfigSpaceList(1000, numberOfParallel))                 ! save all local energies into here 
    ...
    !$OMP PARALLEL SHARED(energyInConfigSpaceList,i , parameterValues) PRIVATE(filename, localenergyInConfigSubspaceList, samplesInConfigSubspace, k, l,m,  iseed, start_time, end_time, localenergyres , flattened_configurationAndParams  ) 
        ...
        write(filename, "(A22, I0)")  "thermalisation_samples", 2
    ...
    meanOfSquared = SUM(energyInConfigSpaceList **2)/sizeofenergyarray
