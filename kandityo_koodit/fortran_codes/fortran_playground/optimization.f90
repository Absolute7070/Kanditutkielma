program optimization 
    ...
    integer, parameter :: numberOfIterations = 1000, numberOfConfig = 1000, numberOfParticles = 2     ! number of iterations and number of configurations 
    ...
    integer, parameter :: numberOfParallel =  1 ! number of cores to be used, default to at least 4 
    integer, parameter :: intervalForThermals = 200 ! interval for the next thermalisation 
    integer :: iseed 


    ! optimization related 
    real(rn), dimension(numberOfParams) :: parameterValues = [1.5599963180524090, 0.90346024910020106  ]  
    ...
    contains 
        ...
        real(rn) function twoPointDiff(configurationspace,  params, whichParameter)
            ...
            !$OMP PARALLEL SECTIONS 
            !$OMP SECTION 
            call mcenergy(configurationspace, parameters_plush,energy1 )               ! E(alpha+h)
            !$OMP SECTION 
            call mcenergy(configurationspace, parameters_minush,energy2 )              ! E(alpha-h )
            !$OMP END PARALLEL SECTIONS 
