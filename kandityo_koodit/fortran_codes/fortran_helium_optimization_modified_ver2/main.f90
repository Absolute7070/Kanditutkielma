program main
    ...
    integer, parameter :: numberOfIterations = 10, numberOfConfig = 1000, numberOfParticles = 2     ! number of iterations across whole configuration space and number of configurations 
    ...
    real(rn), dimension(numberOfParams) :: parameterValues = [ -4.0206598061705026E-002,    4.0002190878145845] 

    ! coordinate value range 
    real(rn), dimension(2), parameter :: coordinateValueRange = [-1, 1]



    ! thermalisation related 
    integer, parameter :: numberOfParallel = 23 ! number of parallel subconfiguration spaces for Metropolis
    integer :: iseed 
    ...
    real(rn) :: mean,   variance, stderrorofmean, sizeofenergyarray 
    ...
    allocate(energyInConfigSpaceList(numberOfParallel))
    ...
    !$OMP PARALLEL SHARED(energyInConfigSpaceList,i) PRIVATE(localenergyInConfigSubspaceList, samplesInConfigSubspace, k, l, iseed, start_time, end_time, localenergyres , flattened_configurationAndParams  ) 
        ...
        Configurationdo: DO k=1, numberOfConfig 
            ...
            if (numberOfConfig == numberOfConfig/2) then 
        ...
        !$OMP CRITICAL
            ...
            energyInConfigSpaceList(i) = SUM(localenergyInConfigSubspaceList)/SIZE(localenergyInConfigSubspaceList)
    ...
    ! sample variance 
    variance = 1/(sizeofenergyarray -1 ) * SUM(  (energyInConfigSpaceList- mean)**2_rn  )
